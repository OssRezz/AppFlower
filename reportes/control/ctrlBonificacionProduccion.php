...
$reporte->setTitle("Bonificación");

//Posicion del titulo
$reporte->setCellValue('A1', 'Bonificación de producción');
...
$reporte->setCellValue('I2', 'Bonificación');
...
$count = 3;
$Reporte = $report->bonificacionProduccion($desde, $hasta);
if ($Reporte != null) {
    foreach ($Reporte as $rows) {
        ...
        $reporte->setCellValue('H' . $count, $rows['Promedio']);
        $reporte->setCellValue('I' . $count, $rows['Bonificiacion']);
...
header('Content-Disposition: attachment;filename="bonificaion_Produccion.xls"');
